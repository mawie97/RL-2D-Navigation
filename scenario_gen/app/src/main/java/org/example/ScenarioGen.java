...
//         int N_AGENTS = 2;
//         int N_TARGETS = 3;
//         int N_OBS = 40;        // number of obstacle cells
//         int MIN_MANHATTAN = 5; // min Manhattan distance agent <-> target
//         Long SEED = null;
//         boolean requireAtLeastOneFreeRingAroundAgents = false; // optional safety
//     }
...
//                     }
//                     // cell == OR(hits)
//                     // Implement OR via sum(hits) >= 1  <-> cell, and >=0 when false
//                     // Easier: cell is true if any hit true; we can enforce:
//                     // sum(hits) >= 1 => cell; cell => sum(hits) >= 1 OR (=0 when false)
//                     // To keep it simple: sum(hits) >= 1  and cell == (sum(hits) >= 1)
//                     // We encode with two implications:
...
//                 // hasFreeNeighbor == OR_{(x,y)} [ (ax==x & ay==y) & (exists dir s.t. in-bounds & !isObs[nx][ny]) ]
//                 // That is heavy to encode exactly; we approximate: if agent is at (x,y), then at least one in-bounds neighbor must not be obstacle.
//                 // Use big disjunction: For each (x,y), define lit_xy <-> (ax==x & ay==y & OR free nbr).
...
//                         // orFree <-> OR(freeNbrs): encode as sum(freeNbrs) >= 1
...
//         // --- Search strategy (stable on 4.10.x) ---
...
//         // Variable selector: "first-fail" (smallest domain size), tie-break by name
...
//         }
//         // Deduplicate any accidental obstacle duplicates (shouldn’t happen due to constraints)
...
//         Config c = new Config();
//         // Very light arg parsing: --H=, --W=, --agents=, --targets=, --obs=, --minMan=, --seed=, --ring=1
//         for (String a : args) {
